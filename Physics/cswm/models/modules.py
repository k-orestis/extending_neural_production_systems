...
import sys
sys.path.insert(1, '../..')
from modular_dynamics.utilities.rule_network_contextual import RuleNetwork
...
class CausalTransitionModel(nn.Module):
    ...
    def __init__(self, embedding_dim_per_object, input_dims, hidden_dim, 
                 ...
                 vae=False, gnn=False, nps = False, learn_edges = False, num_graphs = 10, multiplier=1, ignore_action=False,
        ...
        adj_mask = torch.eye(
                self.num_objects, self.num_objects, dtype=torch.bool)
        ...
        if self.modular or self.gnn or self.nps:
        ...
        if self.modular:
          ...
          self.transition_nets = NPS(input_dim=self.embedding_dim,
                action_dim=action_dim,num_objects=num_objects, rule_dim=self.rule_dim, num_rules=1)
        elif self.gnn:
    ...
    def transition(self, state, action):
        ...
        if self.modular:
            ...
            action = torch.argmax(action, dim=1)
            pred_next_state = self.transition_nets.transition(state, action)
        else:
...
class NPS(nn.Module):
    """NPS transition model"""
    #NPS(input_dim=self.embedding_dim,hidden_dim=hidden_dim, action_dim=action_dim,num_objects=num_objects)
    def __init__(self, input_dim, num_rules, action_dim, rule_dim, num_objects):
        super().__init__()
        self.device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
        self.hidden_dim = input_dim + action_dim*num_objects
        ...
        self.rule_network = RuleNetwork(self.hidden_dim, num_objects, input_dim=input_dim, num_rules = num_rules, rule_dim = rule_dim, query_dim = 32, value_dim = 64, key_dim = 32, num_heads = 4, dropout = 0.1, design_config = self.design_config)
          
    def forward(self, state, action, params=None):
        return self.transition(state, action)
        
    def transition(self, states, action):
        batch_size, num_objects, _ =  states.shape
        slot_attr = states.view(-1, self.input_dim)
        #print(action[0])
        action_vec = utils.to_one_hot(
                    torch.as_tensor(action, dtype = torch.int64), self.action_dim*num_objects).repeat(1, num_objects)
        #print(action_vec.shape)
        action_vec = action_vec.view(-1, self.action_dim*num_objects)
        
        #print(action_vec.shape)
        #print(slot_attr.shape)
        # Attach action to each state
        rule_out = slot_attr
        for _ in range(self.k):
            print('rule out ', rule_out.shape)
            print('action vec', action_vec.shape)
            rule_out = rule_out.view(-1, self.input_dim)
            slot_attr = torch.cat([rule_out, action_vec], dim=-1).view(batch_size, num_objects, -1)
            #print(slot_attr.shape) #-> batch_size, num_obj, hidden_dim
            rule_out, _ = self.rule_network(slot_attr)

        # [batch_size, num_obj, hidden_dim]
        return rule_out #slot_attr.view(batch_size, num_objects, -1) 
