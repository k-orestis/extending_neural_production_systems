...
class RuleNetwork(nn.Module):
    #RuleNetwork(self.hidden_dim, num_objects, input_dim=input_dim, num_rules = num_rules, rule_dim = rule_dim, query_dim = 32, value_dim = 64, key_dim = 32, num_heads = 4, dropout = 0.1, design_config = self.design_config)
    def __init__(self, hidden_dim, num_variables, input_dim, num_transforms = 3,  num_rules = 4, rule_dim = 64, query_dim = 32, value_dim = 64, key_dim = 32, num_heads = 4, dropout = 0.1, design_config = None):
        ...
        #edited
        self.variable_rule_select = SelectAttention(hidden_dim, hidden_dim , d_k=32, num_read = num_rules, num_write = num_variables) #, share_query = True, share_key = True)
    ...
    def forward(self, hidden, message_to_rule_network = None, rule_mask = None):
        ...
        if True:
            ...
            print('sel variable' , selected_variable.shape)
        
            rule_mlp_output = self.rule_mlp(selected_variable)
            print('rule mlp out: ', rule_mlp_output.shape)
            #print('target unchanged: ', target_unchanged.shape)
            ...
            print('rule mlp output',(rule_mlp_output.shape))
            print('rule mlp output repeat', rule_mlp_output.repeat(1,num_variables,1).shape)
            print('v mask', v_mask.shape)
            print('shape b', target_unchanged.shape)
