...
class RuleNetwork(nn.Module):
    def __init__(self, hidden_dim, num_variables, input_dim, num_transforms = 3,  num_rules = 4, rule_dim = 64, query_dim = 32, value_dim = 64, key_dim = 32, num_heads = 4, dropout = 0.1, design_config = None):
        ...
        #edited
        self.rule_mlp = GroupMLP(2*hidden_dim, input_dim, num_rules)
    ...
    def forward(self, hidden, message_to_rule_network = None, rule_mask = None):
        ...
        batch_size, num_variables, hidden_dim = hidden.size() # (batch_size, 2, 4)
        ...
        cont_scores = self.variables_select_1(hidden, hidden) # (batch_size, num_variables, num_variables)
        #cont_mask = torch.nn.functional.gumbel_softmax(cont_scores.reshape(batch_size, -1), dim = 1, tau = 1, hard = True)
        #cont_mask = cont_mask.reshape(batch_size, num_variables, num_variables)
        ...
        if True:
            #print('old one')
            variable_mask = torch.sum(mask, dim = 2).unsqueeze(-1) #(b_s, num_variables, 1)
            rule_mask = torch.sum(mask, dim = 1).unsqueeze(-1) #(bs, 1, num_rules)
            #if self.training:
            ...
            # select contextual
            cont_scores = cont_scores[torch.arange(batch_size), torch.argmax(variable_mask_print, dim = 1).detach().cpu().numpy(), :]
            cont_id = torch.argmax(cont_scores, dim = 1).detach().cpu().numpy()
            ...
            # print("#############################")
            target_unchanged = (hidden * (torch.ones((batch_size, num_variables, 1), dtype=int).to(self.device) - variable_mask))[:,:,:self.input_dim]
            selected_rule = (rule_emb_orig * rule_mask).sum(dim = 1)
            primary_slot = (hidden * variable_mask).sum(dim = 1)
            secondary_slot = hidden[torch.arange(batch_size), cont_id, :]
            ...
            v_mask = variable_mask.repeat(1, 1, self.input_dim)

            rule_mlp_output = rule_mlp_output.repeat(1,num_variables,1)*v_mask + target_unchanged
            return rule_mlp_output, 0
