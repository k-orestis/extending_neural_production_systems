...
class GroupMLP(nn.Module):
    def __init__(self, in_dim, out_dim, num):
        super().__init__()
        self.group_mlp1 = GroupLinearLayer(in_dim, 32, num)
        self.group_mlp2 = GroupLinearLayer(32, out_dim, num)
...
class RuleNetwork(nn.Module):
    def __init__(self, hidden_dim, num_variables, num_transforms = 3,  num_rules = 4, rule_dim = 64, query_dim = 32, value_dim = 64, key_dim = 32, num_heads = 4, dropout = 0.1, design_config = None, steps = 1):
        super().__init__()
        ...
        self.secondary_variable_select = SelectAttention(hidden_dim-2, hidden_dim-2 , d_k=32, num_read = 1, num_write = 2, share_query = True, share_key = True)

        self.encoder_transform = nn.Linear(num_variables * hidden_dim, hidden_dim)
        self.rule_mlp = GroupMLP(hidden_dim, 2, num_rules)
    ...
    def forward(self, hidden, message_to_rule_network = None, rule_mask = None, train=True):
        ...
        if train:
            for seq in range(1):
                scores = self.variable_rule_select(rule_emb, hidden) # (batch_size, num_rules, num_variables)
                
                #if (random.random() > 0.9): print(scores[3])
                for var in range(2):
                    var_score = scores[:,:,var].unsqueeze(-1)
                    
                    
                    if self.training:
            
                        mask = torch.nn.functional.gumbel_softmax(var_score.reshape(batch_size, -1), dim = 1, tau = 1, hard = True)
                        self.rule_probabilities.append(mask.clone().reshape(batch_size, num_rules, 1).detach())
                        probs = mask
                        mask = mask.reshape(batch_size, num_rules, 1)
                        stat_mask = torch.sum(mask, dim = 0)
                        mask = mask.permute(0, 2, 1)
                        var_score = var_score.permute(0, 2, 1).float()
                        #if self.use_biases:
                        #	self.biases += stat_mask.detach().cpu().numpy()
            
                        entropy = 1e-4 * torch.sum(probs * torch.log(probs), dim = 1).mean()
                    else:
                        mask = ArgMax().apply(var_score.reshape(batch_size, -1)).reshape(batch_size, num_rules, 1)
                        mask = mask.permute(0, 2, 1)
                        var_score = var_score.permute(0, 2, 1).float()
                        self.rule_probabilities.append(torch.softmax(var_score.reshape(batch_size, -1), dim = 1).reshape(batch_size, 1, num_rules).clone().detach())
                        entropy = 0
                        mask_print = mask
                    ...
                    if True:
                        ...
                        # primary mask
                        variable_mask = torch.zeros((batch_size,3,1), dtype=int,device=self.device)
                        #variable_mask[:,:, var] = torch.ones((batch_size,1), dtype=int,device=self.device) 
                        ...
                        local_rule_app.append(torch.argmax(rule_mask_print, dim = 1).detach().cpu().numpy())
                        self.variable_activation.append(torch.argmax(variable_mask_print, dim = 1).detach().cpu().numpy())
                        self.variable_activation_1.append((torch.ones((batch_size), dtype=int, device=self.device) - torch.argmax(variable_mask_print, dim = 1)).detach().cpu().numpy())   
                        #### SECONDARY
                        
                        if var == 0: cands = hidden[:,1:,:2]
                        elif var == 1: cands = torch.cat((hidden[:,0,:2].unsqueeze(1), hidden[:,2,:2].unsqueeze(1)), dim=1)
                        else: cands = hidden[:,:2,:2]
                        sec_score = self.secondary_variable_select(hidden[:,var,:2].unsqueeze(1), cands) # (b,1,2)
                        secondary_mask = torch.nn.functional.gumbel_softmax(sec_score.reshape(batch_size, -1), dim = 1, tau = 1, hard = True).reshape(batch_size, 1, 2)
                        secondary_mask = secondary_mask.permute(0, 2, 1)
                        secondary_mask = torch.sum(secondary_mask, dim = 2).unsqueeze(-1)
                        
                        #if (random.random() > 0.9): print(sec_score[3])
                        ####
                        
                    
                        selected_rule = (rule_emb_orig * rule_mask).sum(dim = 1)
      
                        primary_slot = hidden[:,var,0:2]
                        secondary_slot = (cands[:,:,0:2] * secondary_mask).sum(dim = 1)
                        ...
                        rule_mlp_output = torch.sum(rule_mlp_output * rule_mask, dim = 1) # (batch_size, 2)
                        if var == 0: out = rule_mlp_output
                        else: out = torch.cat((out, rule_mlp_output), dim=1)
                        #out.append(rule_mlp_output)
                
            self.rule_activation.append(local_rule_app)      
