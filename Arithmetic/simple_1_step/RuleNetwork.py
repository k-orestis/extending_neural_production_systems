    ...
    def forward(self, hidden, message_to_rule_network = None, rule_mask = None):
        ...
        for seq in range(1): # 1 - step
            ...
            if True:
                ...
                # print("#############################")
                target_unchanged = (hidden * (torch.ones((batch_size, 2, 1), dtype=int, device=self.device) - variable_mask)).sum(dim = 1)[:,2:]
                selected_rule = (rule_emb_orig * rule_mask).sum(dim = 1)
                primary_target = (hidden[:, :, 2:] * variable_mask).sum(dim = 1)
                hidden = hidden[:, :, 0:2]
                primary_slot = (hidden * variable_mask).sum(dim = 1)
                secondary_slot = (hidden * (torch.ones((batch_size, 2, 1), dtype=int, device=self.device) - variable_mask)).sum(dim = 1)
                ...
                rule_mlp_output = torch.sum(rule_mlp_output * rule_mask, dim = 1).unsqueeze(1) # (batch_size, 1, 2)
                # rebuild the slots
                slot_1 = torch.cat([rule_mlp_output, primary_target.unsqueeze(1)], dim=2) #(bs, 1, 4)
                slot_2 = torch.cat([secondary_slot, target_unchanged], dim=1).unsqueeze(1) ##(bs, 1, 4)
                hidden = torch.cat([slot_1, slot_2],dim=1)
                
        self.rule_activation.append(local_rule_app)      
        rule_mlp_output = torch.cat((rule_mlp_output, target_unchanged.unsqueeze(1)), dim=2).squeeze(1)
