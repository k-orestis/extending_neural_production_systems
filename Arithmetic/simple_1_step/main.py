...
def eval_epoch(epoch):
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(val_dataloader):
           ...
           ### edit
            slot_1, slot_2, operation, primary = data[:, 0:1, :], data[:, 1:2,:], data[:,2:(2+1), :], data[:, (2+1), :] # (batch_size, 1, 4) 
            
            target_1 = slot_1[: , :, 2:]
            target_2 = slot_2[: , :, 2:]
            p = primary[:,0].int() # (batch_size,)
            t1 = torch.cat((target_1,target_2), dim=2)  # (batch_size, 1, 4)
            t2 = torch.cat((target_2,target_1), dim=2) # (batch_size, 1, 4)
            targets = torch.cat((t1,t2), dim=1) # (batch_size, 2, 4)
            targets = targets[np.arange(slot_1.shape[0]),p,:]
            
            slot_1 = slot_1.squeeze(1) # (batch_size, 2)
            slot_2 = slot_2.squeeze(1) # (batch_size, 2)
            
            out = model(slot_1, slot_2)
            ...
            if (i == 17):
                ...
                ex1 = (slot_1[idx], slot_2[idx], out[idx])
            if (i == 32):
                idx = random.randrange(50)
                ex2 = (slot_1[idx], slot_2[idx], out[idx])
            ###############
    ...
    if epoch % 10 == 0:
        ...
        print(ex1[0].tolist(), ex1[1].tolist())
        print('predicted targets (primary first)')
        print(ex1[2].tolist())
        print('EX2:')
        print('input points and expected targets')
        print(ex2[0].tolist(), ex2[1].tolist())
        print('predicted targets (primary first)')
        print(ex2[2].tolist())
    ############
    print('')
...
def train_epoch(epoch):
    ...
    for i, data in tqdm(enumerate(train_dataloader)):
        ...
        # if epoch > 80:
        #     print("###############################")
        #     print(torch.cat((out,targets), dim=1))
        #     print("###############################")
        loss = objective(out, targets)
