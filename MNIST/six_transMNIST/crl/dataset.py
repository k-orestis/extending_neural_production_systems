...
def batchify(batch):
    ...
    for b in batch:
        ...
        data_tar.append(b[3])
    # print('images shape return getitem: ', torch.stack(images, dim = 0).shape,"Tar_image size: ", torch.stack(data_tar, dim = 0).shape)

    return torch.stack(images, dim = 0), transforms, torch.stack(transform_vector, dim = 0) , torch.stack(data_tar, dim = 0)

def get_dataloaders(num_transforms = 6, transform_length = 3, batch_size = 50, color = True, shuffle = True):
    
    # Create the dataset (60000 images for train and 10000 for test)
    mnist_orig = load_mnist_datasets('../data', normalize=False)
    train_data = mnist_orig['train']    # = (data, label):([imagesPerBatch,1,28,28],[labelsPerBatch,1])
    ...
    if not color:
        train_dataset = BasicTransformDataset(train_data[0], train_data[1], num_transforms = num_transforms, transform_length = transform_length, gen = False, state = "Train")
                                            # train_data = (train_data[0], train_data[1])=(data, label) :([imagesPerBatch,1,28,28],[labelsPerBatch,1])
