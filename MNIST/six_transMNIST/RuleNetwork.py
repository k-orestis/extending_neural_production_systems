...
class RuleNetwork(nn.Module):
	def __init__(self, hidden_dim, num_variables, num_transforms = 3,  num_rules = 4, rule_dim = 64, query_dim = 32, value_dim = 64, key_dim = 32, num_heads = 4, dropout = 0.1, design_config = None):
		...
		self.rule_mlp = GroupMLP(hidden_dim, hidden_dim, num_rules)                         # !!!!!!!!!!!!!!!
...
# _________________________________________________________________________________________________________________________________
		if str(self.design_config['application_option']).split('.')[0] == '3' and str(self.design_config['application_option']).split('.')[3] == '0':
			...
			rule_mask = rule_mask.t()[perm].t().unsqueeze(-1)
...
# 			print("Hidden size:", hidden.size())
			...
			rule_mlp_input = hidden[:,0,:] #(hidden * variable_score.unsqueeze(-1)).sum(dim = 1) #torch.cat((rule_emb_orig, selected_variable), dim = 2)
			
			
# 			There is no primary and contextual 
...
# 			rule_mlp_input = torch.cat((primary, context), dim=1)
			
			rule_mlp_input = rule_mlp_input.unsqueeze(1).repeat(1, rule_mask.size(1), 1)
# 			print("HERE...rule mlp dims:", rule_mlp_input.shape)
