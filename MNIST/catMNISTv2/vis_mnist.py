...
from EncoderDecoder import Model, MNISTModel, MNISTmyModel
...
def get_model_output_and_bce(model, data_loader, args, observation_images = None, put_text = True, label = 'baseline'):
    ...
    for k_ in range(len(observation_images)):

        rat_image = torch.from_numpy(observation_images[k_]).to(args.device).permute(2, 0,1)
        rat_image = rat_image.unsqueeze(0)
        show_images = [rat_image.squeeze(0).permute(1, 2, 0).cpu().numpy() * 255.0]
        ...
        operation_to_name = {0:"rotate_right", 3: "rotate_left", 2:"translate_up",1:"translate_down"}
        operations = [2,3,1,0,2,0]
        #first_operation = list(rule_operation_to_mask.keys())[operations[0]]
        operation_to_paper_name = {'translate_up':[0.20, 'Translate Up', (255,0,0)],
                                    'translate_down': [0.35, 'Translate Down', (0,255,0)],
                                    'rotate_left': [0.20, 'Rotate Left', (0,0,255)],
                                    'rotate_right': [0.25, 'Rotate Right', (255,255,0)]}
        names = []
        ...
        with torch.no_grad():
            for t in tqdm(range(len(operations))):
                ...
                operation_vector = rule_operation_to_mask[operation].unsqueeze(0)
                print(operation)
        ...
        if put_text:
            for i, n in enumerate(names):
                ...
                whiteboard = cv2.putText(whiteboard, n_[1], (int(width * (i + 1) + 40 * i - n_[0] * width), 10), cv2.FONT_HERSHEY_SIMPLEX , 0.6, (255,0,0))

        show_images = show_images
        #cv2.imshow('image', show_images)
        #cv2.waitKey(0)
        #cv2.destroyAllWindows()

        cv2.imwrite('paper_imgs' + '/_'+str(k_)+'.png', show_images)
        #np.concatenate((whiteboard, show_images), axis = 0) 
...
print(model_name_)
#model_dir = 'logs/CRL_RIM-100_1_num_rules_0_rule_time_steps_0-1-False-False/'
...
model_name_ = args.something
print(model_name_)
model_dir = 'logs/CRL_RIM-100_1_num_rules_2_rule_time_steps_1-3-False-False/'

print(args.train_dataset)
image_2,_ = plot_for_multiple_models(model_dir, test_loader, args = args, put_text = True, label = 'Ours')
