...
class BasicTransformDataset(torch.utils.data.Dataset):
    def __init__(self, inputs, targets, num_transforms = 8, transform_length = 3, gen = True, state = "Train"):
        ...
        self.transforms = {'first_digit': lambda q : q[0], # Rotate(60)()
                           'second_digit': lambda q : q[1],  #Rotate(-60)()
                           'third_digit': lambda q: q[2],
                           'first_second': lambda q: torch.concat((q[0,:,:], q[1,:,:]),dim=1),  #Translate(0.2, 0)()
                           'first_third': lambda q: torch.concat((q[0,:,:], q[2,:,:]),dim=1),
                           'second_first': lambda q: torch.concat((q[1,:,:], q[0,:,:]),dim=1),  #Translate(-0.2, 0)()
                           'second_third': lambda q: torch.concat((q[1,:,:], q[2,:,:]),dim=1),
                           'third_first': lambda q: torch.concat((q[2,:,:], q[0,:,:]),dim=1),
                           'third_second': lambda q: torch.concat((q[2,:,:], q[1,:,:]),dim=1),
    ...
    def __getitem__(self, bsize):   # bsize = random image
        ...
        base_image = place_subimage_in_background((64, 64))(input1)     # resize
        ...
        input2f = place_subimage_in_background((64, 64))(input2) 
        input3f = place_subimage_in_background((64, 64))(input3) 
        ...
        im_ = place_subimage_in_background((64, 64))(im_)
        # images.append(im_)
