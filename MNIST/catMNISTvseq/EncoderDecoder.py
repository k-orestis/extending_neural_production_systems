...
from RuleNetwork import MyRuleNetwork
...
class UnFlatten(nn.Module):
    ...
    def forward(self, input):
        return input.view(input.size(0), 64, 11, 11)
...
class Model(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.slot_attention = 1 #SlotAttention(num_slots = 5,
                              #              dim = 512,
                              #              iters = 3)
...
class MNISTmyModel(nn.Module):
    """Encoder from https://openreview.net/pdf?id=ryH20GbRW
    Decoder is not copied
    """

    def __init__(self, args):
        super(MNISTmyModel, self).__init__()
        self.args = args
        ...
        self.operation_rep = nn.Sequential(nn.Linear(15, 64),
                                            nn.ReLU(),
        ...
        if self.num_rules > 0:
            ...
            self.rule_network = MyRuleNetwork(2*self.output_size, 3, num_rules = args.num_rules, rule_dim = args.rule_dim, query_dim = 32, value_dim = 64, key_dim = 32, num_heads = 4, dropout = 0.1, design_config = self.design_config)
    ...
    def forward(self, x, h_prev, inp_transforms, rule_mask = None):
        # print('x in forward: ', len(x))
        # print('x image in forward: ', x[0].shape)

        # print("x[:,0,:]:", x[:,0,:].shape)

        encoded_input1 = [
            encoder(x[:,0]) for encoder in self.encoders
        ]
        encoded_input2 = [
            encoder(x[:,1]) for encoder in self.encoders
        ]
        encoded_input3 = [
            encoder(x[:,2]) for encoder in self.encoders
        ]
        # print('x in forward after encoder : ', len(encoded_input1))
        # print('x image in forward after encoder : ', encoded_input1[0].shape)
        operation = self.operation_rep(inp_transforms).unsqueeze(1)
        # print('operation:', operation.shape)
        encoded_input = torch.cat((encoded_input1[0].unsqueeze(1), encoded_input2[0].unsqueeze(1), encoded_input3[0].unsqueeze(1)), 1)
        encoded_input = torch.cat((encoded_input, operation), dim = 1)


        #print("encoded input size: ", encoded_input.size()) #(50,3,100)
        #print("transforms input size: ", inp_transforms.size())
        ...
        if self.num_rules > 0:
            for step in range(2):
                encoded_input, mask = self.rule_network(encoded_input, step)
                if step == 0:
                    #print(encoded_input.shape, operation.shape)
                    encoded_input = torch.cat((encoded_input, operation), dim = 1)
                #else:
                #    encoded_input = (encoded_input*mask).sum(dim=1).unsqueeze(1)
            #print("after rule network dec in: ", encoded_input.size())
            #print("after rule network mask: ", mask.size())
        ...
        dec_outs = [self.Decoder[0](encoded_input[:,i]) for i in range(3)]
