...
class MyRuleNetwork(nn.Module):
	def __init__(self, hidden_dim, num_variables, num_transforms = 3,  num_rules = 4, rule_dim = 64, query_dim = 32, value_dim = 64, key_dim = 32, num_heads = 4, dropout = 0.1, design_config = None):
		...
		self.rule_activation1 = []
		self.variable_activation1 = []
		self.softmax = []
		self.masks = []
		import math
		rule_dim = rule_dim
		...
		self.variables_select_1 = SelectAttention(hidden_dim, hidden_dim, d_k = 16, num_read = num_variables, num_write = num_variables)
	...
	def forward(self, hidden, step = 0, message_to_rule_network = None, rule_mask = None):
		...
		num_variables-=1
		op = hidden[:,3,:].unsqueeze(1).repeat(1, 3, 1)
		hidden_ = hidden[:,:3,:]
...
# 			print("Rule mask:", rule_mask.shape)
		...
		if step == 0:
			...
			self.rule_activation1.append(torch.argmax(rule_mask_print, dim = 1).detach().cpu().numpy())
...
# 			print("Hidden size:", hidden.size())
		...
		con_score = self.variables_select_1(hidden, hidden,)
		#print("con score, var mask: ",con_score.shape, variable_mask.shape)
		con_mask = torch.matmul(con_score, variable_mask).squeeze(-1)
		con_mask = torch.nn.functional.gumbel_softmax(con_mask, dim = 1, hard = True, tau = 0.5)

		#torch.sum(mask, dim = 2).unsqueeze(-1)
		#con_score = torch.nn.functional.gumbel_softmax(con_score, dim = 1, hard = True, tau = 0.5)
		if step == 0:
			self.variable_activation.append(torch.argmax(variable_mask_print.detach(), dim = 1).detach().cpu().numpy())
		else:
			self.variable_activation1.append(torch.argmax(variable_mask_print.detach(), dim = 1).detach().cpu().numpy())
		...
		target_unchanged = hidden[:,:, :100] * (torch.ones((batch_size, 3, 1), dtype=int, device= self.device)-  variable_mask  )
		#print(target_unchanged.shape)
		primary = (hidden * variable_mask).sum(dim = 1)
		
		context = (hidden * con_mask.unsqueeze(-1)).sum(dim=1)
		...
		#print("rule output: ", rule_mlp_output.shape)
		rule_mlp_output=  (rule_mlp_output.repeat(1,3,1)* variable_mask) + target_unchanged
		#print("final rule input: ", rule_mlp_output.shape)
		return rule_mlp_output, variable_mask
