...
class BasicTransformDataset(torch.utils.data.Dataset):
    def __init__(self, inputs, targets, num_transforms = 8, transform_length = 3, gen = True, state = "Train"):
        ...
        self.gen = gen
        print('inputs shape in init: ', inputs.shape)

        self.transforms = {'first_digit': lambda q : place_subimage_in_background((88, 88))(q), # Rotate(60)()
                           'second_digit': lambda q : place_subimage_in_background((88, 88))(q),  #Rotate(-60)()
                           'third_digit': lambda q: place_subimage_in_background((88, 88))(q),
                           'first_second': lambda q: torch.stack((place_subimage_in_background((88, 88))(torch.cat((q[0,:,:], q[1,:,:]),dim=2)), place_subimage_in_background((88, 88))(q[1]), place_subimage_in_background((88, 88))(q[2])),dim=0),  #Translate(0.2, 0)()
                           'first_third': lambda q: torch.stack((place_subimage_in_background((88, 88))(torch.cat((q[0,:,:], q[2,:,:]),dim=2)), place_subimage_in_background((88, 88))(q[1]), place_subimage_in_background((88, 88))(q[2])),dim=0),
                           'second_first': lambda q: torch.stack((place_subimage_in_background((88, 88))(q[0]), place_subimage_in_background((88, 88))(torch.cat((q[1,:,:], q[0,:,:]),dim=2)), place_subimage_in_background((88, 88))(q[2])),dim=0),  #Translate(-0.2, 0)()
                           'second_third': lambda q: torch.stack((place_subimage_in_background((88, 88))(q[0]), place_subimage_in_background((88, 88))(torch.cat((q[1,:,:], q[2,:,:]),dim=2)), place_subimage_in_background((88, 88))(q[2])),dim=0),
                           'third_first': lambda q: torch.stack((place_subimage_in_background((88, 88))(q[0]), place_subimage_in_background((88, 88))(q[1]), place_subimage_in_background((88, 88))(torch.cat((q[2,:,:], q[0,:,:]),dim=2))),dim=0),
                           'third_second': lambda q: torch.stack((place_subimage_in_background((88, 88))(q[0]), place_subimage_in_background((88, 88))(q[1]), place_subimage_in_background((88, 88))(torch.cat((q[2,:,:], q[1,:,:]),dim=2))),dim=0),
                           'first_second_third': lambda q: torch.stack((place_subimage_in_background((88, 88))(torch.cat((q[0,:,:], q[1,:,:], q[2,:,:]),dim=2)), place_subimage_in_background((88, 88))(q[1]), place_subimage_in_background((88, 88))(q[2])),dim=0),  
                           'first_third_second': lambda q: torch.stack((place_subimage_in_background((88, 88))(torch.cat((q[0,:,:], q[2,:,:], q[1,:,:]),dim=2)), place_subimage_in_background((88, 88))(q[1]), place_subimage_in_background((88, 88))(q[2])),dim=0),
                           'second_first_third': lambda q: torch.stack((place_subimage_in_background((88, 88))(q[0]), place_subimage_in_background((88, 88))(torch.cat((q[1,:,:], q[0,:,:], q[2,:,:]),dim=2)), place_subimage_in_background((88, 88))(q[2])),dim=0),  
                           'second_third_first': lambda q: torch.stack((place_subimage_in_background((88, 88))(q[0]), place_subimage_in_background((88, 88))(torch.cat((q[1,:,:], q[2,:,:], q[0,:,:]),dim=2)), place_subimage_in_background((88, 88))(q[2])),dim=0),
                           'third_first_second': lambda q: torch.stack((place_subimage_in_background((88, 88))(q[0]), place_subimage_in_background((88, 88))(q[1]), place_subimage_in_background((88, 88))(torch.cat((q[2,:,:], q[0,:,:], q[1,:,:]),dim=2))),dim=0),
                           'third_second_first': lambda q: torch.stack((place_subimage_in_background((88, 88))(q[0]), place_subimage_in_background((88, 88))(q[1]), place_subimage_in_background((88, 88))(torch.cat((q[2,:,:], q[1,:,:], q[0,:,:]),dim=2))),dim=0),
    ...
    def __getitem__(self, bsize):   # bsize = random image
        # get the raw data
        transforms = []
        images = []
        imagesf = []
        tar_images =[]
        bsize_ = random.randint(0,4999)
        bsize__ = random.randint(0,4999)
        # inputs, targets = self.inputs[bsize], self.targets[bsize]
        input1, input2, input3 = self.inputs[bsize], self.inputs[bsize_]  , self.inputs[bsize__]      # !!!!!!!!!!!!
        #print('inputs shape in getitem: ', input1.shape) 1, 28, 28
        transforms = []
        # images = [place_subimage_in_background((64, 64))(inputs)]
        base_image = place_subimage_in_background((88, 88))(input1)     # resize
        #print('images shape in getitem: ', base_image.shape) #1, 64, 64
        #print('bsize= ', bsize)
        #transform_vector = []
        ...
        input2f = place_subimage_in_background((88, 88))(input2) 
        input3f = place_subimage_in_background((88, 88))(input3) 
        #images.append(torch.stack((base_image,input2), dim=0))
        images.append(torch.cat((input1,input2, input3), dim=0))
        imagesf.append(torch.stack((base_image,input2f, input3f), dim=0))
        
        transform_vector = torch.zeros(self.num_transforms)
        transform_vector[transform_id] = 1.
        #transform_vectors.append(cur_vector)
        transforms.append(list(self.transforms.keys())[transform_id])
        
        im_ = images[-1].unsqueeze(dim=1)
        im_ = self.transforms[transforms[-1]](im_)  #.unsqueeze(dim=0)
        #im_ = place_subimage_in_background((88, 88))(im_)
        # images.append(im_)
        tar_images.append(im_)
        # ftiaxnw trans, fn dataloader batchify, run_crl, 
        #transform_vectors = torch.stack(transform_vectors, dim = 0)     # list to tensor
        #images = torch.stack(imagesf, dim = 0)
        tar_images = torch.stack(tar_images, dim = 0)
        #print('images shape return getitem: ', images.shape,"Tar_image size: ", tar_images.shape)
        return imagesf[0], transforms, transform_vector, im_ # 4*2*64*64, 4*64*64
