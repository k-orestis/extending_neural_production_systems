...
class CompositeTransformDataset(torch.utils.data.Dataset):
    def __init__(self, inputs, targets, num_transforms = 8, transform_length = 3, gen = True, state = "Train", iter = 2):
        """     
            inputs: (bsize, 1, H, W)
            targets: (bsize, 1)
            transformation_combination: TransformationCombiner object
        ...
        self.transforms = {'rotate_right': lambda x: Rotate(10*x)(), #x 0 -> 90
                           'rotate_left': lambda x: Rotate(-10*x)(),
                           'translate_up':lambda x: Translate(x*0.05, 0)(), #x 0 -> 0.4
                           'translate_down':lambda x: Translate(-x*0.05, 0)(),
    ...
    def __getitem__(self, bsize):
        ...
        if transform_id == 0 or transform_id == 1:
            t = random.randint(4, 8)
        else:
            t = random.randint(4, 8)


        transform_vector[transform_id] = t
        trans.append(list(self.transforms.keys())[transform_id])
        im_ = images[-1]
        im_ = self.transforms[trans[-1]](t)(im_)
        images.append(im_)
        
        #transform_vectors = torch.stack(transform_vectors, dim = 0)
        #images = torch.stack((images[0]), dim = 0)
        #im_ = torch.stack((images[1]), dim = 0)
        return images[0], trans, transform_vector, images[1]
