...
from .dataloader.mnist_transforms import shrink_mnist_dataset, cuda_mnist_dataset, \
    TransformationCombinationDataLoader, BasicTransformDataset,CompositeTransformDataset, TrainTransformDataset, ColorTransformDataset
...
def batchify(batch):
    ...
    for b in batch:
        ...
        tar_images.append(b[3])
    return torch.stack(images, dim = 0), transforms, torch.stack(transform_vector, dim = 0), torch.stack(tar_images, dim = 0)
...
def get_dataloaders(num_transforms = 4, transform_length = 3, batch_size = 50, color = True, shuffle = True):
    ...
    if not color:
        train_dataset = CompositeTransformDataset(train_data[0], train_data[1], num_transforms = num_transforms, transform_length = transform_length, gen = False, state = "Train")

        val_dataset = CompositeTransformDataset(val_data[0], val_data[1], num_transforms = num_transforms, transform_length = transform_length, gen = False, state = "Val")    

        test_dataset = CompositeTransformDataset(test_data[0], test_data[1], num_transforms = num_transforms, transform_length = transform_length, gen = False, state = "Val")
    else:
        
        train_dataset = CompositeTransformDataset(train_data[0], train_data[1], num_transforms = num_transforms, transform_length = transform_length, gen = True, state = "Train")

        val_dataset = CompositeTransformDataset(val_data[0], val_data[1], num_transforms = num_transforms, transform_length = transform_length, gen = True, state = "Val")    

        test_dataset = CompositeTransformDataset(test_data[0], test_data[1], num_transforms = num_transforms, transform_length = transform_length, gen = True, state = "Val")
