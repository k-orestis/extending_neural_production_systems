...
from RuleNetwork import CompRuleNetwork
...
class CompMNISTModel(nn.Module):
    """Encoder from https://openreview.net/pdf?id=ryH20GbRW
    Decoder is not copied
    """

    def __init__(self, args):
        super(CompMNISTModel, self).__init__()
        self.args = args
        ...
        if self.num_rules > 0:
            ...
            #self.rule_network = CompRuleNetwork(2*self.output_size, 1, num_rules = args.num_rules, rule_dim = args.rule_dim, query_dim = 32, value_dim = 64, key_dim = 32, num_heads = 4, dropout = 0.1, design_config = self.design_config)
            self.rule_network = CompRuleNetwork(self.output_size, 2, num_rules = args.num_rules, rule_dim = args.rule_dim, query_dim = 32, value_dim = 64, key_dim = 32, num_heads = 4, dropout = 0.1, design_config = self.design_config)
    ...
    def forward(self, x, h_prev, inp_transforms, rule_mask = None):
        ...
        # before operation = self.operation_rep(inp_transforms)
        operation = self.operation_rep(inp_transforms).unsqueeze(1)
        encoded_input = torch.cat(encoded_input, 1)
        encoded_input = torch.cat((encoded_input.unsqueeze(1), operation), dim = 1)
        # before encoded_input = torch.cat((encoded_input, operation), dim = 1).unsqueeze(1)

        #print("encoded input size: ", encoded_input.size())
        ...
        if self.num_rules > 0:
            encoded_input, mask = self.rule_network(encoded_input, inp_transforms)
        ...
        dec_outs = self.Decoder[0](encoded_input)
