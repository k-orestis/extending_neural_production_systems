...
class CompRuleNetwork(nn.Module):
	def __init__(self, hidden_dim, num_variables, num_transforms = 3,  num_rules = 4, rule_dim = 64, query_dim = 32, value_dim = 64, key_dim = 32, num_heads = 4, dropout = 0.1, design_config = None):
		...
		#self.rule_mlp = GroupMLP((hidden_dim//2) + 10, hidden_dim//2, num_rules)
		self.rule_mlp = GroupMLP((hidden_dim) + 10, hidden_dim, num_rules)
		...
		self.mnist_entity_selector = SelectAttention(rule_dim, hidden_dim, d_k = 16, num_read = 1, num_write = 2)
	...
	def forward(self, hidden, inp_transforms=None, message_to_rule_network = None, rule_mask = None):
		...
		if str(self.design_config['application_option']).split('.')[0] == '3' and str(self.design_config['application_option']).split('.')[3] == '0':
			...
			self.variable_activation.append(torch.argmax(variable_mask.detach(), dim = 1).detach().cpu().numpy())
			#rule_mlp_input = hidden[:,:,:100]
			rule_mlp_input = hidden[:,0].unsqueeze(1) #(hidden * variable_score.unsqueeze(-1)).sum(dim = 1) #torch.cat((rule_emb_orig, selected_variable), dim = 2)
			inp_transforms = torch.max(inp_transforms, dim=1)[0].repeat(10,1).t()
			rule_mlp_input = torch.cat((rule_mlp_input, inp_transforms.unsqueeze(1)), dim = -1)
			#print("rule mlp inp before: ", rule_mlp_input.shape)
			rule_mlp_input = rule_mlp_input.repeat(1, rule_mask.size(1), 1)
