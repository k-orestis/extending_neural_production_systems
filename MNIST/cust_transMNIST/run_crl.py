...
def train_loop(model, train_loader, optimizer, epoch, logbook,
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        if a==3:
            ...
            index = lexi[data[1][0][0]]
            #print(inp_transforms[0,index].item())
            plt.savefig('logs/images/'+str(batch_idx)+data[1][0][0] +str(inp_transforms[0,index].item())[:3]+'.png')
...
def test_loop(model, test_loader, epoch, transfer_loader, logbook,
    ...
    for i, data in enumerate(test_loader):
        if args.num_rules > 0:
            model.rule_network.reset_activations()
        #print("len, data0, data2 ", len(data), data[0].shape, data[2].shape, data[3].shape)
        inp_data = data[0].to(args.device)
        #target = target[:,0]
        inp_transforms = data[2].to(args.device)
        loss = 0
