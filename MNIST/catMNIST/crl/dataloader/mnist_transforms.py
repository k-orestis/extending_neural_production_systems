...
class BasicTransformDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, bsize):   # bsize = random image
        ...
        input1, input2 = self.inputs[bsize], self.inputs[bsize_]        # !!!!!!!!!!!!
        ...
        for t in range(self.transform_length):
            ...
            bsize_ = random.randint(0,4999)
            input2 = self.inputs[bsize_]
            input2f = place_subimage_in_background((64, 64))(input2) 
            #images.append(torch.stack((base_image,input2), dim=0))
            images.append(torch.cat((input1,input2), dim=0))
            imagesf.append(torch.stack((base_image,input2f), dim=0))
            """
            else:
                #if self.state == "Train":
        ...
        return images, transforms, transform_vectors, tar_images # 4*2*64*64, 4*64*64
