...
class MNISTmyModel(nn.Module):
    ...
    def __init__(self, args):
        ...
        if self.num_rules > 0:
            ...
            self.rule_network = MyRuleNetwork(2*self.output_size, 2, num_rules = args.num_rules, rule_dim = args.rule_dim, query_dim = 32, value_dim = 64, key_dim = 32, num_heads = 4, dropout = 0.1, design_config = self.design_config)
    ...
    def forward(self, x, h_prev, inp_transforms, rule_mask = None):
        ...
        encoded_input = torch.cat((encoded_input1[0].unsqueeze(1), encoded_input2[0].unsqueeze(1)), 1)
