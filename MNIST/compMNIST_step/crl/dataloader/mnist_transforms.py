...
class CompositeTransformDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, bsize):
        ...
        transform_vector = torch.zeros(2*self.num_transforms)
        for t in range(self.iter):
            ...
            """
                    ...
                    if targets % 2 == 0:
                        ...
                        transform_id = random.randint(0, 1)
            """
            transform_vector[t*self.num_transforms + transform_id] = 1.
            trans.append(list(self.transforms.keys())[transform_id])
            transforms += (list(self.transforms.keys())[transform_id])
            im_ = images[-1]
            im_ = self.transforms[trans[-1]](im_)
            images.append(im_)
        
        #transform_vectors = torch.stack(transform_vectors, dim = 0)
        images = torch.stack((images[0],images[2]), dim = 0)
        return images, transforms, transform_vector
