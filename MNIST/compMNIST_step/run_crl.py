...
def train_loop(model, train_loader, optimizer, epoch, logbook,
    ...
    lexi = {0: 'rotate_right',
        ...
        2: 'translate_up',
        3: 'translate_down'}
    value_to_index = {value: index for index, value in enumerate([i+j for i in lexi.values() for j in lexi.values()])}


    # Initialize the confusion matrix as an 8x8 matrix filled with zeros
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        start_time = time()
        #print("len, data0, data2 ", len(data), data[0].shape, data[2].shape)
        ...
        inp_data_ = inp_data
        inp_data = inp_data_[:, 0]
        tar_data = inp_data_[:, 1]

        
        b, c, w, h = inp_data.size()
        ...
        if a==3:
            plt.imshow(inp_data[0,0].detach().cpu().numpy())
            plt.savefig('logs/images/'+str(batch_idx)+'1st.png')
            plt.imshow(out[0,0].detach().cpu().numpy())
            plt.savefig('logs/images/'+str(batch_idx)+'out.png')
            plt.imshow(tar_data[0,0].detach().cpu().numpy())
            plt.savefig('logs/images/'+str(batch_idx)+data[1][0] +'.png')
            rule_selections = model.rule_network.rule_activation
            a = np.array(rule_selections).T
            predicted_list = [lexi[i[0]] + lexi[i[1]] for i in a]
            b_ = data[2][:,:4].detach().numpy()
            b2_ = data[2][:,4:].detach().numpy()
            #print(b_, b2_)
            a_ = np.array([np.argmax(b_, axis = 1), np.argmax(b2_, axis = 1)], dtype=np.int32).T
            true_list =[lexi[i[0]] + lexi[i[1]] for i in a_]
    
            # Iterate through each pair of true and predicted values and update the confusion matrix
            for true_value, predicted_value in zip(true_list, predicted_list):
                true_index = value_to_index[true_value]
                predicted_index = value_to_index[predicted_value]
                confusion_matrix[true_index][predicted_index] += 1

            print("should be a perm matrix: ", confusion_matrix)
        ...
        #print(data[1])
        for t in data[1]:
            entire_trasnform_list.append(t)
        ...
        if args.num_rules > 0:
            ...
            #print(rule_selections, entire_trasnform_list)
...
def test_loop(model, test_loader, epoch, transfer_loader, logbook,
    ...
    for i, data in enumerate(test_loader):
        ...
        loss = 0
        kl_loss = 0
        entire_transform_list = []
        for t in data[1]:
            entire_transform_list.append(t)

        hidden = torch.zeros(1, args.batch_size, args.hidden_size).squeeze().to(args.device)
        #hidden = GruState(hidden)
        #for frame in range(inp_data.size(1) - 1):
        #output, hidden, extra_loss, block_mask, _, _ = model(inp_data[:, frame, :, :, :], hidden, inp_transforms[:, frame, :])
        output = model(inp_data[:, 0, :, :, :], None, inp_transforms)
        target = inp_data[:, 1, :, :, :]
        #print(target.shape)
        #print(output.shape)
        loss = loss_fn(output, target)
        ...
        if args.num_rules > 0:
            ...
            #print(rule_selections, variable_selections)
            #if i % 100 == 0:
            #    print(rule_probs)
            #rule_variable = get_stats(rule_selections, variable_selections, rule_variable, args.application_option, args.num_rules, args.num_blocks)
            #print(len(rule_selections))
            #print(len(data[1][0]))
            #rule_to_transform = rule_transform_stats(rule_selections, entire_transform_list, rule_to_transform, args)
