...
class CompMNISTModel(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.operation_rep = nn.Sequential(nn.Linear(8, 64),
                                            nn.ReLU(),
    ...
    def forward(self, x, h_prev, inp_transforms, rule_mask = None):
        ...
        encoded_input = torch.cat((encoded_input, operation), dim = 1).unsqueeze(1)
        ...
        if self.num_rules > 0:
            self.rule_network.rule_activation = []
            for step in range(2):
                encoded_input, mask = self.rule_network(encoded_input, step)
                #print("encoded input size: ", encoded_input.size())
                if step == 0:
                    encoded_input = torch.cat((encoded_input.squeeze(1), operation), dim = 1).unsqueeze(1)
