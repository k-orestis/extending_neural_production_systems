...
class CompRuleNetwork(nn.Module):
	...
	def forward(self, hidden, step=0, message_to_rule_network = None, rule_mask = None):
		...
		if str(self.design_config['application_option']).split('.')[0] == '3' and str(self.design_config['application_option']).split('.')[3] == '0':
			...
			con = torch.full((batch_size, 1, 10), step, device=self.device)
			#(hidden * variable_score.unsqueeze(-1)).sum(dim = 1) #torch.cat((rule_emb_orig, selected_variable), dim = 2)
			#print("rule mlp inp before: ", rule_mlp_input.shape)
			rule_mlp_input = torch.cat((hidden[:,:,:100],con), dim = 2)
			rule_mlp_input = rule_mlp_input.repeat(1, rule_mask.size(1), 1)
			#print("rule mlp inp: ", rule_mlp_input.shape)
